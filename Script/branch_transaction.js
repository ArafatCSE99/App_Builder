...
var branchid=document.getElementById("branchids").value;
...
$.ajax({
type: "POST",
url: "Model/getbranchPreviousBalance.php",
data: dataString,
cache: false,
async:false,
...
today=dates;
       ...
       if(branchid=="" || previous_balance=="" || amount=="" || current_balance=="" || transaction_type=="" ||  dates=="" )
       {
       ...
       if(id==0){
        var sql="INSERT INTO app_branch_account (`userid`,companyid, `branchid`, `previous_balance`, `in_account`, `out_account`, `balance`, `transaction_date`) VALUES ('"+userid+"','"+companyid+"','"+branchid+"','"+previous_balance+"','"+in_account+"','"+out_account+"','"+current_balance+"','"+today+"')";
        //alert(sql);
       }
       else{
        var sql = "UPDATE app_branch_account SET branchid='"+branchid+"', previous_balance='"+previous_balance+"',in_account='"+in_account+"',current_balance='"+current_balance+"',out_account='"+out_account+"' WHERE id="+id;
       }
...
function updatedata(updateid,e)
{
  ...
  $('#branchids').find("option:contains(" + $branchid+ ")").attr('selected', 'selected');
  $('#branchids').trigger('change');
