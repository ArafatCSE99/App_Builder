...
if($userid==1 || $userid==4 || $userid==10  ){
$sql = "SELECT * FROM warehouse_product where (userid=1 or userid=4 or userid=10) ";
}
else
{
$sql = "SELECT * FROM warehouse_product where userid=$userid";
}
...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
    ...
    $id=$row["id"];
    $name=$row["name"];
    $export_userid=$row["export_userid"];
    if($row["image"]!="")
    {
...
if ($resultc->num_rows > 0) {
  ...
  while($rowc = $resultc->fetch_assoc()) {
    $warehousename=$rowc["name"];
     ...
     echo "<td class='sales_price'>".$row['sales_price']."</td>"; 
     echo "<td class='sales_price_smrp'>".$row['sales_price_smrp']."</td>"; 
     ...
     echo "<td class='text-center py-0 align-middle' style='text-align:center;'>
                      <div class='btn-group btn-group-sm'>
                        ...
                        <a onclick=deletedata($id,this,'warehouse_product') class='btn btn-danger'><i class='fas fa-trash'></i></a>
                      </div>
                    </td>";
     }
     else
...
$sqlc = "SELECT * FROM user where id=$export_userid";
$resultc = $conn->query($sqlc);
