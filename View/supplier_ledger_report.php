...
<br>
      ...
      <div class = "form-group"  id='customerDiv'>
            <label class = "sr-only" for = "">Supplier</label>
...
echo "<option value='' hidden=''>Select Supplier</option>";
...
<?php if($s_customer!=""){ 
 ...
 $sqls = "SELECT name,mobileno,opening_due FROM supplier where id=$s_customer";
...
?>
  ...
  <div class="row">
    ...
    <div class="col-sm-8" >
       ...
       <center><h5 style="margin-top:5px; margin-bottom:10px;"><b><?php echo "Supplier Ledger Report" ?></b><h5></span></center>
       <?php 
         if($s_dtfrom!="")
         {
...
$slno=0;
     ...
     $Previous_Balance=$opening_due==""?0:$opening_due;
     $Current_Balance=0;

$sql = "SELECT Transaction_Date,Type,Amount FROM
(
  SELECT purchase_date as Transaction_Date,'Purchase Due' as Type,due as Amount FROM purchase_master where userid=$userid and supplierid=$s_customer and due>0
  union all
  SELECT transaction_date as Transaction_Date,'Given(Payment)' as Type,out_account as Amount FROM app_supplier_account where userid=$userid and supplierid=$s_customer and out_account>0
  union all
  SELECT transaction_date as Transaction_Date,'Taken' as Type,in_account as Amount FROM app_supplier_account where userid=$userid and supplierid=$s_customer and in_account>0
  union all
  SELECT payment_date as Transaction_Date,'Purchase Payment' as Type,amount as Amount FROM purchase_payment_inv where  purchaseid in(select id from purchase_master where supplierid=$s_customer)
...
if($Type=="Purchase Due" || $Type=="Taken")
{
    $Current_Balance=$Previous_Balance+$Amount;
}
else
{
    //echo $Current_Balance." ".$Previous_Balance." ".$Amount."<br>";
