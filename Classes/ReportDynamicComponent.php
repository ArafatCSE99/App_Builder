...
class DynamicComponent
{
    ...
    private function createTextbox($label, $value, $class,$column_name,$required)
    {
        return "
            <div class='{$class}'>
                <input type='text' name='{$label}' id='{$column_name}' value='{$value}' placeholder='$label' style='margin-left:10px;' class='form-control' $required>
            </div>
        ";
    }
    ...
    private function createDropdown($label, $value, $class, $tablename, $value_column, $option_column,$column_name,$required,$onchnge_table,$onchange_value_column,$onchange_option_column)
    {
        ...
        $dropdown = "
            <div class='{$class}'>
                <select name='{$label}' id='{$column_name}'  style='margin-left:10px;' class='form-control'  $required>
        ";
        }
        else{
            $dropdown = "
            <div class='{$class}'>
                <select name='{$label}' id='{$column_name}'  style='margin-left:10px;' onchange='getDepndentData(this,\"$onchnge_table\",\"$onchange_value_column\",\"$onchange_option_column\")' class='form-control'  $required>
        "; 
        }
