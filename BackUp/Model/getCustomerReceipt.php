...
$sql = "SELECT * FROM basic_info where userid=(select userid from customer where id=$customerid) and companyid=$companyid";
...
$sql = "SELECT * FROM sales_master where customerid=$customerid limit 1";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
...
<table id="" class="table" style="width:100%; font-size:15px;" >
...
$sql = "SELECT * FROM sales_detail where salesid in (Select id from sales_master where customerid=$customerid)";
$result = $conn->query($sql);
...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
    ...
    // Get master Data .........................................
$sqlj = "SELECT * FROM sales_master where id=$salesid";
$resultj = $conn->query($sqlj);
if ($resultj->num_rows > 0) {
  while($rowj= $resultj->fetch_assoc()) {
      $sales_date=$rowj["sales_date"];
      $grand_total=$grand_total+$rowj["grand_total"];
      $paid=$paid+$rowj["paid"];
      $due=$due+$rowj["due"];
      $master_discount=$master_discount+$rowj["discount"];
      $note=$rowj["note"];
  }
} 
...
?>
                  ...
                  <tfoot style="background-color:white; font-weight: bold;">
                     <tr> <td style='text-align:right;' colspan="3"><td style='text-align:right;' colspan="3">Total Qty : <?php echo $total_qty ?></td><td style='text-align:right;'>Discount : </td> <td><?php echo $master_discount ?></td> </tr>
                     <tr> <td style='text-align:right;' colspan="7">Grand Total : </td> <td><?php echo $grand_total ?></td> </tr>
                     <tr> <td style='text-align:right;' colspan="7">Paid : </td> <td><?php echo $paid ?></td> </tr>
                     <tr> <td style='text-align:right;' colspan="7">Due : </td> <td><?php echo $due ?></td> </tr>
                     
                     <?php if($totalpay>0){ ?>
                     <tr> <td style='text-align:right;' colspan="7">Payment : </td> <td><?php echo $totalpay ?></td> </tr>
                     <tr> <td style='text-align:right;' colspan="7">Current Due : </td> <td><?php echo $currentdue ?></td> </tr>
                     <?php } ?>
