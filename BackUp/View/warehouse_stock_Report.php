...
if ($resultse->num_rows > 0) {
    ...
    ?>
         ...
         &nbsp;&nbsp;
          ...
          <div class = "form-group">
            <label class = "sr-only" for = "dtfrom">Warehouse</label>
...
$resultse = $conn->query($sqlsem);
echo "<option value='' hidden=''>All Warehouse</option>";
if ($resultse->num_rows > 0) {
...
if($up_name!="" && $dis_name!="")
{
  ...
  <div class="row">
    ...
    <div class="col-sm-9" style="padding-left:20px;">
       ...
       <h5 style="margin-top:5px; margin-bottom:10px;"><b><?php echo "Stock Report (Warehouse)" ?></b><h5></span>
...
$sql = "SELECT distinct b.id,b.name as model,a.name FROM category a,product b  where a.companyid=b.companyid and a.id=b.categoryid and b.userid=$userid and b.companyid=$companyid and (b.categoryid='$s_ctg' or '$s_ctg'='') and (b.id='$s_model' or '$s_model'='' ) ";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
  // output data of each row
  while($row = $result->fetch_assoc()) {
...
$current_stock=0;
// Get Previous_Qty .......
$sqlc = "SELECT sum(current_stock) as sum_current_stock FROM stock_warehouse where userid=$userid and companyid=$companyid and productid=$id and (warehouseid='$s_code' or '$s_code'='')";
...
if ($resultc->num_rows > 0) {
  ...
  while($rowc = $resultc->fetch_assoc()) {
    ...
    $current_stock=$rowc["sum_current_stock"];
  }
} else {
...
// Get purchase .......
$sqlc = "SELECT * FROM purchase_detail where productid in (select id from product where id=$id) and purchaseid in (select id from purchase_master where (warehouseid='$s_code' or '$s_code'='') ) and 
purchaseid in (
...
if ($resultc->num_rows > 0) {
  ...
  while($rowc = $resultc->fetch_assoc()) {
    ...
    //$msrp_price=$rowc["msrp_price"];
    //$total_price=$rowc["total_price"];
