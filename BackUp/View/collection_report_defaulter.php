...
if($up_name!="" && $dis_name!="")
{
 ...
 <div class="row">
    ...
    <div class="col-sm-9" style="padding-left:20px;">
       ...
       <h5 style="margin-top:5px; margin-bottom:10px;"><b><?php echo "Collection Report" ?></b><h5></span>
       <?php 
         if($s_dtfrom!="")
         {
...
$sql = "SELECT a.*,b.*,a.discount as pay_discount FROM sales_payment a,sales_master b where a.salesid=b.id and b.userid=$userid and b.companyid=$companyid and b.branchid=$branchid
    and b.customerid in (Select id from customer where isDefaulter='checked')
    order by b.customerid,a.payment_date";
//echo $sql;
$result = $conn->query($sql);
if ($result->num_rows > 0) {
  // output data of each row
  while($row = $result->fetch_assoc()) {
    ...
    $mobileno="";
    $reference="";
    $collection_amount=$row["pay_amount"];
...
if(($dt>=$s_dtfrom && $dt<=$s_dtto) or $s_dtfrom==""){
     ...
     echo "<td class='payment_date'>".$row["payment_date"]."</td>";
     echo "<td class='total_price'>".$row["pay_amount"]."</td>";
     echo "<td class='total_price'>".$row["pay_discount"]."</td>";
     ...
     $Total_payment=$Total_payment+$collection_amount;
...
echo "<td class='' colspan='4'><b>Grand Total</b></td>";
     ...
     echo "<td class=''><b>".$Total_discount."</b></td>";
