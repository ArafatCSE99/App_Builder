...
if ($resultse->num_rows > 0) {
    ...
    ?>
         ...
         &nbsp;&nbsp;
         <div class = "form-group">
            <label class = "sr-only" for = "dtfrom">Visit By</label>
...
$resultse = $conn->query($sqlsem);
echo "<option value='' hidden=''>Visit By</option>";
if ($resultse->num_rows > 0) {
...
if($up_name!="" && $dis_name!="")
{
  ...
  <div class="row">
    ...
    <div class="col-sm-9" style="padding-left:20px;">
       ...
       <h5 style="margin-top:5px; margin-bottom:10px;"><b><?php echo "Visit Report" ?></b><h5></span>
       <?php 
         if($s_dtfrom!="")
         {
...
$sql = "SELECT id,name,mobileno,address,reference,image,opening_due,month,employeeid,visiting_date,note FROM visiting_info where userid=$userid and companyid=$companyid and branchid=$branchid and (employeeid=$s_ctg or $s_ctg=0)";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
  // output data of each row
  while($row = $result->fetch_assoc()) {
